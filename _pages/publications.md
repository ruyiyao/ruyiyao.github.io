---
layout: archive
title: "Publications"
permalink: /publications/
author_profile: true
---

{% if site.author.googlescholar %}
  <div class="wordwrap">You can also find my articles on <a href="{{site.author.googlescholar}}">my Google Scholar profile</a>.</div>
{% endif %}

<!--
- **[IWQoS'25] Empowering Flowlet Load Balancing in RDMA with Host-Based Flowlet Fine-Tuning**  
  IEEE/ACM International Symposium on Quality of Service (IWQoS), 2025  
  Chuhao Chen, Deli Huang, Zerui Tian, ***Ruyi Yao***, Jing Jiang, Sen Liu, and Yang Xu.
-->


- **[SIGCOMM'23] BMW Tree: Large-scale, High-throughput and Modular PIFO Implementation using Balanced Multi-Way Sorting Tree**  
  ACM SIGCOMM Conference on Data Communication, 2023  
  ***Ruyi Yao***, Zhiyu Zhang, Gaojian Fang, Peixuan Gao, Sen Liu, Yibo Fan, Yang Xu, and H. Jonathan Chao.  

- **[SIGCOMM'24] vPIFO: Virtualized Packet Scheduler for Programmable Hierarchical Scheduling in High-Speed Networks**  
  ACM SIGCOMM Conference on Data Communication, 2024  
  Zhiyu Zhang, Shili Chen, ***Ruyi Yao***, Ruoshi Sun, Hao Mei, Hao Wang, Zixuan Chen, Gaojian Fang, Yibo Fan, Wanxin Shi, Sen Liu, and Yang Xu.

- **[SIGCOMM'25] CClinguist: An Expert-Free Framework for Future-Compatible Congestion Control Algorithm Identification**  
  ACM SIGCOMM Conference on Data Communication, 2025  
  Jiahui Li, Han Qi, ***Ruyi Yao***, Jialin Wei, Ruoshi Sun, Zixuan Chen, Sen Liu, and Yang Xu.

- **[INFOCOM'23] CoLUE: Collaborative TCAM Update in SDN Switches**  
  IEEE International Conference on Computer Communications (INFOCOM), 2023  
  ***Ruyi Yao***, Cong Luo, Hao Mei, Chuhao Chen, Wenjun Li, Ying Wan, Sen Liu, Bin Liu, and Yang Xu.

- **[ICNP'21] MagicTCAM: A Multiple-TCAM Scheme for Fast TCAM Update**  
  IEEE International Conference on Network Protocols (ICNP), 2021  
  ***Ruyi Yao***, Cong Luo, Xuandong Liu, Ying Wan, Bin Liu, Wenjun Li, and Yang Xu.

- **[IWQoS'25] Empowering Flowlet Load Balancing in RDMA with Host-Based Flowlet Fine-Tuning**  
  IEEE/ACM International Symposium on Quality of Service (IWQoS), 2025  
  Chuhao Chen, Deli Huang, Zerui Tian, ***Ruyi Yao***, Jing Jiang, Sen Liu, and Yang Xu.

- **[IWQoS'24] TCAMVisor: High-throughput TCAM Virtualization for Multi-tenant Software Defined Networking**  
  IEEE/ACM International Symposium on Quality of Service (IWQoS), 2024  
  Ruoshi Sun, Hao Mei, ***Ruyi Yao***, Hao Wang, Yiren Zhou, Zixuan Chen, Sen Liu, and Yang Xu.  
  *Best Paper Honorable Mention.*

- **[IWQoS'23] Rusen: Rule Semantics Enabler toward Fast TCAM Update for Commodity SDN Switches**  
  IEEE/ACM International Symposium on Quality of Service (IWQoS), 2023  
  Hao Mei, Ruoshi Sun, ***Ruyi Yao***, Chuhao Chen, Cong Luo, Zixuan Chen, Jiahui Li, Sen Liu, and Yang Xu.

- **[IWQoS'22] BubbleTCAM: Bubble Reservation in SDN Switches for Fast TCAM Update**  
  IEEE/ACM International Symposium on Quality of Service (IWQoS), 2022  
  Cong Luo, Chuhao Chen, Hao Mei, ***Ruyi Yao***, Ying Wan, Wenjun Li, Sen Liu, Bin Liu, and Yang Xu.

- **[ICNP'21] PIPO: Efficient Programmable Scheduling for Time Sensitive Networking**  
  IEEE International Conference on Network Protocols (ICNP), 2021  
  Chuwen Zhang, Zhikang Chen, Haoyu Song, ***Ruyi Yao***, Yang Xu, Yi Wang, Ji Miao, and Bin Liu.

- **[ANCS'21] KickTree: A Recursive Algorithmic Scheme for Packet Classification with Bounded Worst-Case Performance**  
  ACM/IEEE Symposium on Architectures for Networking and Communications Systems (ANCS), 2021  
  Yao Xin, Yuxi Liu, Wenjun Li, ***Ruyi Yao***, Yang Xu, and Yi Wang.

- **[Neurocomputing'21] Routing Optimization Meets Machine Intelligence: A Perspective for the Future Network**  
  Neurocomputing, 2021  
  Bin Dai, Yuanyuan Cao, Zhongli Wu, Zhewei Dai, ***Ruyi Yao***, and Yang Xu.

- **[CCF ToN'20] Packet-size aware scheduling algorithms in guard band for time sensitive networking**  
  CCF Transactions on Networking, 2020  
  Chuwen Zhang, Yi Wang, ***Ruyi Yao***, Boyang Zhou, Liang Cheng, Yang Xu, Xiaoguang Li, Jian Cheng, and Bin Liu.
